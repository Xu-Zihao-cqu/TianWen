...
│               │   │   │
│               │   │   ├── [WorkDetailPage]
│               │   │   │   ├── WorkHeader  |（内联组件，定义于 WorkDetailPage.jsx 文件内，非独立文件）
...
```
> **图标约束**：`icon` 字段存 Lucide 图标名的 kebab-case 字符串（如 `'cpu'`、`'code-2'`）。
> 渲染时需通过一个集中的图标映射表将字符串转为组件，
> 该映射表定义在 `src/utils/icons.js`，按需维护。
> 
> ```js
> // src/utils/icons.js
> import { Cpu, Code2, Share2, BookOpen, Github, Video, Mail } from 'lucide-react';
> 
...
| 用途 | 字体 | 加载方式 |
|------|------|----------|
| 中文正文 | Noto Sans SC (思源黑体) | 自托管（woff2 子集，放 public/fonts/） |
| 英文正文 | Inter          | 自托管（woff2 子集，放 public/fonts/）|
| 英文标题 | Space Grotesk  | 自托管（woff2 子集，放 public/fonts/）|
| 代码     | JetBrains Mono | 自托管（woff2 子集，放 public/fonts/）|

> ⚠️ **国内网络注意**：Google Fonts CDN 在中国大陆可能被墙，导致字体加载失败。v1 建议将 Noto Sans SC（思源黑体）自托管（下载 woff2 子集放入 `public/fonts/`），四款字体均自托管，从 Google Fonts 下载对应语言子集的 woff2 文件，放入 public/fonts/ 并在 fonts.css 中通过 @font-face 引入。推荐仅下载 latin + latin-ext 子集（Inter/Space Grotesk/JetBrains Mono），文件体积合计约 300KB，可接受。（`system-ui`, `-apple-system`, `Microsoft YaHei` 等），保证国内环境下仍有可读的排版效果。
...
- **语言检测优先级**：localStorage → navigator.language（经规范化：zh-CN/zh-TW → zh）→ 默认 'zh'
...
function useI18n() {
  ...
  const t = (obj) => {
    if (typeof obj === 'object' && obj !== null && ('zh' in obj || 'en' in obj)) {
      return obj[i18n.language] !== undefined ? obj[i18n.language] : obj.zh;
    }
    return obj;
  };
...
备选:     yourusername/tianwen-files    ← 若单仓库过大，独立文件仓库
          → 通过 Raw URL 引用
          ⚠️ LFS 额度预警：GitHub 免费账户 LFS 存储上限 1GB / 带宽 1GB/月。若文件总量接近 1GB，应提前迁移到 Cloudflare R2 或 Backblaze B2，将 public/files/ 下的本地路径改为对象存储公开 URL，前端代码无需其他改动。
```
