...
| 5 | `index.html` | HTML 入口；英文字体已自托管，无需 Google Fonts preconnect；仅保留 favicon 引用和 lang 属性。；lang 属性；favicon 引用。Noto Sans SC 走自托管，不需要 Google Fonts preconnect。
| 6 | `src/main.jsx` | React 入口，渲染 `<App />` |
| 7 | `src/styles/index.css` | Tailwind 三指令 + CSS 变量（色板、阴影、圆角） |
| 8 | `src/styles/fonts.css` | `@font-face` 定义，`display=swap`；四款字体（Noto Sans SC / Inter / Space Grotesk / JetBrains Mono）均通过@font-face 自托管，display: swap，各引用对应 woff2 子集文件。，避免 Google Fonts 在大陆被墙导致中文字体加载失败（详见 TopDesign §7.2）|
...
| 11 | vercel.json | SPA 路由重写规则，内容为 {"rewrites":[{"source":"/(.*)", "destination":"/index.html"}]}，防止刷新页面或直接访问子路由时 Vercel 返回 404 |
...
生产依赖:
  ...
  react-markdown
  react-pdf
  # react-pdf 依赖 pdfjs-dist，Worker 文件通过 Vite ?url 导入，无需手动复制到 public/
  prismjs
...
| # | 文件 | 说明 |
|---|------|------|
| 0 | src/App.jsx | 完整 Provider 嵌套骨架已就位（HelmetProvider / ErrorBoundary / I18nextProvider / ThemeProvider）；| I18nextProvider 和 ThemeProvider 在 Step 4 填充实现，AnimatePresence 在 Step 11 激活，| 此步骤先 import 并挂空壳，保证结构正确，避免后续步骤忘记集成 |
...
| 11 | src/contexts/AuthContext.jsx | v2 预留空壳：导出空的 AuthContext 和 AuthProvider（直接透传 children），占位不实现逻辑 |

### 路由配置

```jsx
// App.jsx 核心结构
<HelmetProvider>               {/* react-helmet-async；SEO 标签生效的前提 */}
  <ErrorBoundary>              {/* 全局错误捕获；Step 5 中创建，Step 2 先挂空壳占位即可 */}
    <I18nextProvider i18n={i18n}>   {/* Step 4 完成后补实现 */}
      <ThemeProvider>               {/* Step 4 完成后补实现 */}
        <BrowserRouter>
          <ScrollToTop />
          <AnimatePresence mode="wait">   {/* Step 11 完成后补实现 */}
            <Routes>
...
| 2.5 | `src/utils/icons.js` | Lucide 图标名字符串到组件的映射表；初始收录 categories / skills / externalLinks 用到的全部图标 |
...
| 7 | src/data/seo.js | 全局 SEO 默认值（title、description、ogImage、siteName），Step 12 中各页面直接 import 使用 |
...
|   |                       | navigator.language 可能返回 'zh-CN'、'zh-TW' 等带区域后缀的值，
|   |                       | 需规范化：lang.startsWith('zh') ? 'zh' : 'en'，
|   |                       | 确保与数据文件中 { zh, en } 键名严格匹配 |
...
| # | 文件 | 说明 |
|---|------|------|
| 1 | `src/components/features/WorkCard.jsx` | 作品卡片，接受 `compact` prop（默认 false）：
|   |                                       | 标准模式（compact=false）：封面图 + 标题 + 短描述 + 标签组 + FileTypeBadge，用于列表页；
|   |                                       | 精简模式（compact=true）：封面图 + 标题 + 前两个标签，隐藏描述，固定宽度 280px，
|   |                                       | 用于首页 FeaturedWorks 横向滚动展示 |
...
- **动态渐变色**：动态渐变色：categories.js 中的 gradient 字段直接存 CSS 色值对象 { from, to }，渲染时使用内联样式：style={{ backgroundImage: `linear-gradient(135deg, ${category.gradient.from}, ${category.gradient.to})` }}不使用 Tailwind 渐变工具类，避免 JIT 编译时无法识别动态类名的问题。
...
**依赖**：Step 3（作品数据）+ Step 5（Tag、Card、Badge、EmptyState、LazyImage）+ Step 7（WorkCard）+ React Router useSearchParams（Step 2 已引入，此处直接使用）。
...
| 4 | `src/pages/WorksListPage.jsx` | 组装 PageHeader + TagFilter + WorkCardGrid / EmptyState | 同步 URL ?tags= 查询参数（使用 React Router useSearchParams，读取初始值 + 写回选中状态）
...
| 2 | `src/components/features/PDFViewer.jsx` | react-pdf 封装，分页导航（上一页/下一页/页码），加载状态 | ；需在组件顶部配置 pdfjs Worker：import workerSrc from 'pdfjs-dist/build/pdf.worker.min.mjs?url' + GlobalWorkerOptions.workerSrc = workerSrc（Vite 环境必须加 ?url 后缀）|
| 3 | `src/components/features/CodeViewer.jsx`   | Prism.js 封装，语法高亮 + 复制按钮；通过文件扩展名自动推断语言（.py→python / .js→javascript / .c→c 等）；亮/暗模式通过 prism-theme.css 中两套选择器切换，无需 JS 干预 |
...
| 7 | `src/pages/WorkDetailPage.jsx` | 组装以上全部 + WorkHeader（标题+标签）+ 返回按钮 | WorkHeader（内联于 WorkDetailPage.jsx） | 作品标题 + 标签列表 + 返回按钮；
|     |                                        | 因逻辑简单、复用率为零，不单独成文件，
|     |                                        | 直接作为 WorkDetailPage 内部子组件定义在同一文件顶部 |
...
所有路由页面正常显示
直接输入公网 URL 访问 /works/hardware 或在 /works/hardware/hw-sample-1 页面刷新，页面能够正常加载，不触发 Vercel 的默认 404 页面
亮暗切换、语言切换在生产环境正常工作
图片、文件资源加载正常
...
## 2026-09-07 增量：五板块与 Blog

在原 13 步已实现的网站上，按数据 → 路由/页面 → 动效 → 构建 → 推送的顺序增量实施：
1. STM32 网关数据与封面；硬件列表第一位。
2. Blog 第五板块与数据库服务、RLS 迁移。
3. 列表、详情、Developer 编辑预览与发布；路由懒加载。
4. 首页主视觉、响应式导航与精选布局；减少动效适配。
5. 构建与输入边界验证后推送 GitHub。数据库操作按用户指示排除，详见 blog-setup.md。

### Blog 多媒体增量实施

1. 文件类型/大小/数量约束、Storage 服务与迁移。
2. 代码、PDF、Word 渲染组件及安全清理。
3. 编辑器上传/封面/草稿、阅读页附件与目录、列表视觉。
4. 构建与边界测试后推送；不操作线上 Supabase。
